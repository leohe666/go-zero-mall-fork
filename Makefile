...
# 用法: make run        # 启动 5 个 air (每服务独立) + start-services.sh 进程守护
#      make build       # 仅编译 5 个服务到 tmp/
#      make air         # 仅跑 5 个 air 监听编译 (后台)
#      make services    # 仅跑 start-services.sh 进程守护
#      make logs        # 查看指定服务的 air 输出: make logs svc=user_rpc
#      make stop        # 停止所有后台进程
#      make clean       # 清理编译产物
...
# 5 个服务对应的 air 配置 (4 个 RPC 服务 + 1 个统一网关; 旧 API 服务已下线)
AIR_CONFIGS := .air.user_rpc.toml .air.product_rpc.toml .air.order_rpc.toml .air.pay_rpc.toml \
...
help:
	@echo "可用命令:"
	@echo "  make run       - 启动开发环境 (5个 air 独立盯每服务 + 进程守护)"
	@echo "  make build     - 仅编译 5 个服务到 tmp/"
	@echo "  make air       - 仅运行 5 个 air 监听文件变化并编译 (后台)"
	@echo "  make services  - 仅运行进程守护 (需 air 在后台跑)"
	@echo "  make logs      - 查看指定服务 air 输出: make logs svc=user_rpc"
...
# 启动完整开发环境: 5 个 air (后台, 输出各自日志) + start-services.sh (前台)
run:
...
# 仅编译 5 个服务
build:
	@echo "=== Building 5 services to tmp/ ==="
	@cd $(CODE_DIR) && \
	...
	echo "Build done: 5 binaries in tmp/"

# 仅跑 5 个 air (后台编译触发器)
air:
	@echo "=== Running 5 air file watchers (per-service) ==="
	@cd $(CODE_DIR) && \
	mkdir -p tmp && \
	for cfg in $(AIR_CONFIGS); do \
	  name=$${cfg#.air.}; name=$${name%.toml}; \
	...
	echo "All 5 air watchers running in background"; \
	echo "Run 'make services' in another terminal to start services"

# 查看指定服务的 air 输出 (实时): make logs svc=user_rpc
logs:
	@cd $(CODE_DIR) && if [ -n "$(svc)" ]; then \
	  tail -f tmp/air.$(svc).log; \
	else \
	  echo "用法: make logs svc=<服务名>  如: make logs svc=user_rpc"; \
	  echo "可用的 air 日志:"; \

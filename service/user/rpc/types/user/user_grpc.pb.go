...
func (c *userClient) LoginByCasdoor(ctx context.Context, in *LoginByCasdoorRequest, opts ...grpc.CallOption) (*LoginByCasdoorResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(LoginByCasdoorResponse)
	err := c.cc.Invoke(ctx, User_LoginByCasdoor_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _User_LoginByCasdoor_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LoginByCasdoorRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UserServer).LoginByCasdoor(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: User_LoginByCasdoor_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UserServer).LoginByCasdoor(ctx, req.(*LoginByCasdoorRequest))
	}
	return interceptor(ctx, in, info, handler)
}
